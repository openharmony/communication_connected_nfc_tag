...
public:
    ...
    ErrCode UnRegisterUserCallBack(const sptr<INfcTagCallback> &callBack);
    ...
    bool IsUserCallbackEmpty();

private:
    int RemoteOnNotify(MessageParcel &data, MessageParcel &reply);
    // one process, many callbacks
    std::vector<sptr<INfcTagCallback>> callbackStore_;
